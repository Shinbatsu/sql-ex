WITH xxx AS(SELECT DISTINCT _.maker,
...
ELSE 'no'END printer FROM(SELECT ttt.maker,
ttt.pc,(SELECT count(DISTINCT pc.model)FROM product _
JOIN pc ON pc.model = _.model
WHERE _.maker = ttt.maker) z,
ttt.laptop,
(SELECT count(DISTINCT l.model)FROM product _
JOIN laptop l ON l.model = _.model
WHERE _.maker = ttt.maker) x,ttt.printer,
(SELECT count(DISTINCT p.model)FROM product _
JOIN printer p ON _.model = p.model
WHERE _.maker = ttt.maker) c
FROM xxx ttt) asdasdd
